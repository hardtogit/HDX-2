	...
	methods: {
		...
		init() {
			...
			if (this.data.listData.length === 0) {
				this.setData({list: [], itemWrapHeight: 0,visibleArr:[false]});
				return;
			}
			...
			if(_addVisible.length==0){
				_addVisible.push(false)
			}
			this.setData({
				visibleArr:_addVisible
			})
		...
		onAddEvent(params){
            if(params.detail.type==='visible'){
			const _visibleArr=	this.data.visibleArr.map((item,index)=>{
					if(index!==params.detail.data.index){
						return false
					}else{
						return !params.detail.data.visible
					}
				})
                this.setData({
                    visibleArr:_visibleArr
                })
			}
